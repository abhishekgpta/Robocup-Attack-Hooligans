...
bool
RoleSideBack::PassToPoint( PlayerAgent * agent, int receiver )
{
...
bool
RoleSideBack::doPass( PlayerAgent * agent )
{
    return SamplePlayer().PassToBestPlayer(agent);
}

int 
RoleSideBack::ClosestPlayerToBall(PlayerAgent * agent){
    double mindis = 20;
...
bool
RoleSideBack::IsOccupiedForPassing(PlayerAgent * agent, int ourPl){
