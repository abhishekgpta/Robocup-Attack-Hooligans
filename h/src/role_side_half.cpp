...
bool
RoleSideHalf::PassToPoint( PlayerAgent * agent, int receiver )
{
...
bool
RoleSideHalf::doPass( PlayerAgent * agent )
{
    return SamplePlayer().PassToBestPlayer(agent);
}

int 
RoleSideHalf::ClosestPlayerToBall(PlayerAgent * agent){
    double mindis = 20;
...
bool
RoleSideHalf::IsOccupiedForPassing(PlayerAgent * agent, int ourPl){
